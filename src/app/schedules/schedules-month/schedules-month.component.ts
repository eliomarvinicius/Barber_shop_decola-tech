...
export class SchedulesMonthComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.subscriptions.push(this.clientHttpService.list().subscribe((data: SelectClientModel[]) => this.clients = data))
  }
  ...
  private fetchSchedules(currentDate: Date) {
    ...
    this.subscriptions.push(this.httpService.listInMonth(year, month).subscribe((data: any) => this.monthSchedule = data));
  }
