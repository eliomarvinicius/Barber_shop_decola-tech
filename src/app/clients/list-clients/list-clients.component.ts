...
export class ListClientsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.httpSubscriptions.push(this.httpService.list().subscribe((data: ClientModelTable[]) => this.clients = data))
  }
  ngOnDestroy(): void {
  ...
  delete(client: ClientModelTable) {
    this.httpSubscriptions.push(
      this.httpService.delete(client.id).subscribe((_: any) => this.snackBarManager.show(`O cliente ${client.name} foi excluido com sucesso`))
    )
  }
